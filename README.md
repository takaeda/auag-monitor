...
### Q: プラチナ (PL=F) や銅 (HG=F) にも対応しますか？

**A: しません。**

このプロジェクトは金と銀の**比率分析**に特化しています。プラチナを追加すると「じゃあパラジウムは？」「ロジウムは？」となり、作者がめんどくさくなります。

金銀比率は数千年の歴史を持つ指標です。それ以上でも、それ以下でもない。Au/Ag。それで完結。

もし他の金属が見たければ、`collector.py` の `SYMBOLS` リストに追加してください。コードは汎用的に書いてあるので動くはずです。でも、**サポートはしません**ｗ
